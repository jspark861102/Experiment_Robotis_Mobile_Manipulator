...
bool Dynamixel::init(uint32_t baud_rate)
{
  ...
  // if (dxl_wb_.scan(&get_dxl_id[0], &dxl_info_.size, 10))
  // {
  ...
  {
    dxl_wb_.ping(id, &dxl_model_num);
    dxl_id_.push_back(id);
  }

  LOG::INFO("ID : " + String(dxl_id_.at(0)));
  LOG::INFO("ID : " + String(dxl_id_.at(1)));
  LOG::INFO("ID : " + String(dxl_id_.at(2)));
  LOG::INFO("ID : " + String(dxl_id_.at(3)));
  LOG::INFO("ID : " + String(dxl_id_.at(4)));
...
{
  setCurrentBasedPositionControlMode(15, 30);
