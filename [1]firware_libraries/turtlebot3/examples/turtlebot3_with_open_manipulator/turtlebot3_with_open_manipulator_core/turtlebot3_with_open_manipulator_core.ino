...
void loop()
{
  ...
  //controllers.getRCdata(&open_manipulator, goal_velocity_from_rc100, goal_time_joint_gripper, RCinput);
  controllers.getRCdata(goal_velocity_from_rc100, goal_time_joint_gripper, RCinput);
...
void motorPowerCallback(const std_msgs::Bool& power_msg)
{
  ...
  motor_driver.setTorque(left_rear_wheel_id_, dxl_power);
  motor_driver.setTorque(right_rear_wheel_id_, dxl_power);
  motor_driver.setTorque(left_front_wheel_id_, dxl_power);
  motor_driver.setTorque(right_front_wheel_id_, dxl_power);
...
void publishSensorStateMsg(void)
{
  ...
  bool dxl_comm_result1 = false;
  bool dxl_comm_result2 = false;
  bool dxl_comm_result3 = false;
  bool dxl_comm_result4 = false;

  int32_t left_rear_wheel_encoder;
  int32_t right_rear_wheel_encoder;
  int32_t left_front_wheel_encoder;
  int32_t right_front_wheel_encoder;
  ...
  //dxl_comm_result1 = motor_driver.readEncoder(left_rear_wheel_id_, sensor_state_msg.left_rear_encoder);
  //dxl_comm_result2 = motor_driver.readEncoder(right_rear_wheel_id_, sensor_state_msg.right_rear_encoder);
  //dxl_comm_result3 = motor_driver.readEncoder(left_front_wheel_id_, sensor_state_msg.left_front_encoder);
  //dxl_comm_result4 = motor_driver.readEncoder(right_front_wheel_id_, sensor_state_msg.rigjt_front_encoder);

  dxl_comm_result1 = motor_driver.readEncoder(left_rear_wheel_id_, left_rear_wheel_encoder);
  dxl_comm_result2 = motor_driver.readEncoder(right_rear_wheel_id_, right_rear_wheel_encoder);
  dxl_comm_result3 = motor_driver.readEncoder(left_front_wheel_id_, left_front_wheel_encoder);
  dxl_comm_result4 = motor_driver.readEncoder(right_front_wheel_id_, right_front_wheel_encoder);
  sensor_state_msg.left_encoder = (left_rear_wheel_encoder + left_front_wheel_encoder)/2;
  sensor_state_msg.right_encoder = (right_rear_wheel_encoder + right_front_wheel_encoder)/2;
  
  if (dxl_comm_result1 == true  && dxl_comm_result2 == true && dxl_comm_result3 == true && dxl_comm_result4 == true)
    //updateMotorInfo(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder);
    updateMotorInfo(left_rear_wheel_encoder, right_rear_wheel_encoder, left_front_wheel_encoder, right_front_wheel_encoder);
  else
    return;
...
void updateJointStates(void)
{
  ...
  for (uint8_t w_num = 0; w_num < WHEEL_NUM; w_num++)
  {
    joint_states_pos[w_num]  = last_rad[w_num];
    joint_states_vel[w_num]  = last_velocity[w_num];    
  }
...
bool calcOdometry(double diff_time)
{
  ...
  double wheel_lr, wheel_rr, wheel_lf, wheel_rf;      // rotation value of wheel [rad]
  ...
  //wheel_l = wheel_r = 0.0;
  wheel_lr = wheel_rr = wheel_lf = wheel_rf = 0.0;
 ...
 // delta_s     = WHEEL_RADIUS * (wheel_r + wheel_l) / 2.0;
  delta_s     = WHEEL_RADIUS * (wheel_lr + wheel_rr + wheel_lf + wheel_rf) / 4.0;
...
*******************************************************************************/
void jointControl_RC100(void)
{   
  double gripper_position_RC100[1] = {0.0};
  if (RCinput == true)
  {
    manipulator_driver.writeJointProfileControlParam(3.0f, 0.75f);
    manipulator_driver.writeGripperProfileControlParam(0.0f);
    manipulator_driver.writeJointPosition(goal_time_joint_gripper);  
    manipulator_driver.writeGripperPosition(gripper_position_RC100);
...
void jointControl_mod(void)
{
  if (joint_trajectory_point.data_length == 5)
  {
    jointControl_teleop();    
  }
  else
...
void jointControl_teleop(void)
{
  if (is_moving == true)
  {
    ...
    double goal_mod[joint_cnt];
    for (uint32_t mod_i = 0+1; mod_i < 5; mod_i++)
    {        
      goal_mod[mod_i-1] = joint_trajectory_point.data[mod_i];
    }
    manipulator_driver.writeJointProfileControlParam(joint_trajectory_point.data[0], 0.75f);
    manipulator_driver.writeJointPosition(goal_mod);
...
void driveTest(uint8_t buttons)
{
  ...
  motor_driver.readEncoder(left_rear_wheel_id_, current_tick[LEFT_REAR]);
  motor_driver.readEncoder(right_rear_wheel_id_, current_tick[RIGHT_REAR]);
  motor_driver.readEncoder(left_front_wheel_id_, current_tick[LEFT_FRONT]);
  motor_driver.readEncoder(right_front_wheel_id_, current_tick[RIGHT_FRONT]);
  ...
  if (move[LINEAR])
  {    
    if (abs(saved_tick[RIGHT_FRONT] - current_tick[RIGHT_FRONT]) <= diff_encoder)
    {
...
void initJointStates(void)
{
  ...
  static char *joint_states_name[] = {"wheel_left_rear_joint", "wheel_right_rear_joint", "wheel_left_front_joint", "wheel_right_front_joint", "joint1", "joint2", "joint3", "joint4", "gripper"};
...
void sendDebuglog(void)
{
  ...
  motor_driver.readEncoder(left_rear_wheel_id_, encoder[LEFT_REAR]);
  motor_driver.readEncoder(right_rear_wheel_id_, encoder[RIGHT_REAR]);
  motor_driver.readEncoder(left_front_wheel_id_, encoder[LEFT_FRONT]);
  motor_driver.readEncoder(right_front_wheel_id_, encoder[RIGHT_FRONT]);
