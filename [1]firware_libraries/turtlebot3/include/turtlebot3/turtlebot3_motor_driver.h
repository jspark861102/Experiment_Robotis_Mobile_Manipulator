...
#define WAFFLE_DXL_LIMIT_MAX_VELOCITY            300     // MAX RPM is 70 when XM is powered 12.0V
...
class Turtlebot3MotorDriver
{
 public:
  ...
  bool readEncoder(uint8_t id, int32_t &value);
  //bool writeVelocity(int64_t left_value, int64_t right_value);
  bool writeVelocity(int64_t lr_value, int64_t rr_value, int64_t lf_value, int64_t rf_value);
  
  //bool controlMotor(const float wheel_radius, const float wheel_separation, float* value);
  bool controlMotor(const float wheel_radius, const float separation_x, const float separation_y, float* value);
